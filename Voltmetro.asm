...
inicio:
    ...
    movlw   0x01
    movwf   TRISA
    movlw   b'00000111'     ; Switches en PORTE (RE0, RE1, RE2)
    movwf   TRISE
    movlw   0x0E            ; RA0=Analogo, RE0,1,2=Digital
    movwf   ADCON1
    
    bcf     STATUS,5
    
    movlw   b'11000001'     ; Fosc/32, CH0, ADC ON
    movwf   ADCON0
...
leer_adc:
    ...
    call    ret200      ; Espera (retardo de adquisición)
    btfsc   ADCON0,2    ; Espera a que termine (bit GO/DONE = 0)
    goto    $-1
    movf    ADRESH,W    ; Lee los 8 bits más significativos
    movwf   adc_resultado
    return
;----------------------------------------------------------------
mostrar_binario:
...
;       Basado en el "Hola Mundo" que SÍ funcionó:
;       E (Enable) = RD0
;       RS         = RD1
;----------------------------------------------------------------
comando:
    movwf   PORTB       ; 1. Poner comando en PORTB
    call    ret200
    bcf     PORTD,1     ; 2. RS = 0 (Comando) <-- CORREGIDO A RD1
    bsf     PORTD,0     ; 3. E = 1 (Pulso de subida) <-- CORREGIDO A RD0
    call    ret200
    bcf     PORTD,0     ; 4. E = 0 (Pulso de bajada) <-- CORREGIDO A RD0
    call    ret200
    return
;----------------------------------------------------------------
datos:
    movwf   PORTB       ; 1. Poner dato en PORTB
    call    ret200
    bsf     PORTD,1     ; 2. RS = 1 (Dato) <-- CORREGIDO A RD1
    bsf     PORTD,0     ; 3. E = 1 (Pulso de subida) <-- CORREGIDO A RD0
    call    ret200
    bcf     PORTD,0     ; 4. E = 0 (Pulso de bajada) <-- CORREGIDO A RD0
    call    ret200
